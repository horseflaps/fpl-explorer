...
.header{background:linear-gradient(135deg,#0f172a,#1e293b);color:white;padding:32px 40px;}
.header h1{margin:0 0 4px;font-size:24px;color:#00ff87;font-weight:900;}
.header .sub{color:#94a3b8;font-size:12px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;}
.meta{margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;}
.meta span{background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);padding:5px 12px;border-radius:8px;font-size:12px;color:#e2e8f0;font-weight:600;}
...
.content{padding:28px 40px;line-height:1.7;font-size:13px;}
h2{color:#0f172a;font-size:15px;font-weight:800;margin:22px 0 8px;padding-bottom:5px;border-bottom:2px solid #00ff87;}
strong{font-weight:700;}li{margin:3px 0;}p{margin:6px 0;}
.footer{margin-top:32px;padding:14px 40px;background:#f8fafc;border-top:1px solid #e2e8f0;font-size:11px;color:#94a3b8;display:flex;justify-content:space-between;}
</style></head><body>
...
async function sendAutopilotEmail(email, displayName, teamName, gameweek, analysisText, pdfBuffer, executionResult) {
    ...
    const attachments = pdfBuffer ? [{ filename: `wolf-gw${gameweek}-${teamName.replace(/\s+/g, '-')}.pdf`, content: pdfBuffer.toString('base64'), type: 'application/pdf' }] : [];
