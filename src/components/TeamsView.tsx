            ...
            {activeTeam ? (
                <div className="space-y-6">
                    <div className="bg-slate-950 rounded-2xl p-8 border border-slate-800 text-center relative overflow-hidden">
                        ...
                        <p className="text-gray-400 font-mono text-sm uppercase tracking-widest relative z-10">Short Code: {activeTeam.short_name}</p>
                            ...
                            {loading ? (
                                ...
                                <div className="space-y-3">
                                    {recentResults.map(fix => {
                                        ...
                                        let result = 'D', color = 'text-gray-400';
                                        ...
                                        return (
                                            <div key={fix.id} className="flex items-center justify-between text-sm border-b border-slate-800 pb-2 last:border-0">
                                                ...
                                                <div className="text-center font-bold text-white w-1/3">{scoreHome} - {scoreAway}</div>
                                                <div className="text-right text-gray-400 w-1/3 truncate">vs {opponent?.short_name} ({isHome ? 'H' : 'A'})</div>
                                            </div>
                                        );
                                    })}
                                    ...
                                    {nextFixtures.map(fix => {
                                        ...
                                        return (
                                            <div key={fix.id} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-1 h-8 rounded-full ${isHome ? 'bg-fpl-green' : 'bg-fpl-blue'}`} />
                                                    <div>
